...
// Adapted by Simul from the CIniFile class written by Adam Clauss
// You may use this class/code as you wish in your programs.	Feel free to distribute it, and
// email suggested changes to: cabadam@tamu.edu.
...
namespace simul
{
	namespace base
	{
		SIMUL_BASE_EXPORT_CLASS IniFile	
		{
		private:
			bool	 caseInsensitive;
			std::string path;
			SIMUL_BASE_EXPORT_STRUCT key {
		...
		public:
			///
			enum errors{ noID = -1};
			IniFile( const char *iniPath = NULL);
			virtual ~IniFile();
			...
			void Path(std::string const newPath)								{path = newPath;ReadFile();}
			...
			bool HasValue( std::string const keyname, std::string const valuename) const;
			...
			{
				if(GetValue(keyname,valuename)=="true")
					return true;
				if(GetValue(keyname,valuename)=="false")
					return false;
				if(GetValue(keyname,valuename)=="TRUE")
					return true;
				if(GetValue(keyname,valuename)=="FALSE")
