...
ScenePhyKinematic::ScenePhyKinematic( int SCR_WIDTH, int SCR_HEIGHT, PhysicsEngine* physicsEngine, Input* _input )
{
	...
	planePhy->rigidBody->setCollisionFlags( planePhy->rigidBody->getCollisionFlags() | btCollisionObject::CF_KINEMATIC_OBJECT );
	planePhy->rigidBody->setActivationState( DISABLE_DEACTIVATION );
}

void ScenePhyKinematic::Update( float deltaTime )
{
	k += deltaTime;
	planePhy->transform->position( glm::vec3( 0.0f, k, 0.0f ) );

	btTransform btTrans;
	btTrans.setFromOpenGLMatrix( glm::value_ptr( planePhy->transform->pose ) );
	planePhy->rigidBody->getMotionState()->setWorldTransform( btTrans );
