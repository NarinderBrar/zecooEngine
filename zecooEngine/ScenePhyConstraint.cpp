#include "ScenePhyConstraint.h"

ScenePhyConstraint::ScenePhyConstraint( int SCR_WIDTH, int SCR_HEIGHT, PhysicsEngine* physicsEngine, Input* _input )
{
	...
	btRigidBody* rb = cubePhy->rigidBody;
	btTypedConstraint* p2p = new btPoint2PointConstraint( *rb, btVector3( 0.5, 0.5, 0.5 ) );
	phyEng->dynamicsWorld->addConstraint( p2p );
	p2p->setBreakingImpulseThreshold( 10.2 );
}

void ScenePhyConstraint::Update( float deltaTime )
{
...
void ScenePhyConstraint::Render()
{
